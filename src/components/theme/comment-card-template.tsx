...
const CommentCardTemplate: React.FC<CommentCardProps> = ({ comments }) => {
  return (
    <Card>
        ...
        {comments.map((comment, index) => (
          <div key={index} className="py-4">
            <div className="flex items-center gap-5">
              <h5 className="text-gray text-lg md:text-2xl font-bold">
                {comment.name}
              </h5>
              <Badge className={comment.badgeColor}>{comment.status}</Badge>
            </div>
            <div className="p-2 md:p-4">
              <p className="text-philippine-silver font-normal md:font-bold text-sm md:text-xl">
                {comment.message}
              </p>
            </div>
          </div>
