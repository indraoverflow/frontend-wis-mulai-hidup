...
import {
  createInvitationFormToRequest,
  invitationResponseToForm,
} from "@/lib/utils/create-invitation-util";
import { useRouter } from "next/navigation";
import { Alert, AlertDescription } from "@/components/ui/alert";
import { RxCrossCircled } from "react-icons/rx";
import { ArrowUp } from "lucide-react";

export default function UpdateInvitationFormPage({
  uniqueId,
}: {
  uniqueId: string;
}) {
  ...
  useEffect(() => {
    form.reset(invitationResponseToForm(data?.data));
  }, [data?.data]);
  ...
  return !data?.data ? (
    <p className="w-screen  flex items-center justify-center">Loading</p>
  ) : (
    <div className="min-h-screen bg-surface md:py-24">
      ...
      <FormProvider {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
          ...
          {formIndex === 3 && (
            <AdditonalInformationForm form={form} isUpdate={true} />
          )}
        </form>
